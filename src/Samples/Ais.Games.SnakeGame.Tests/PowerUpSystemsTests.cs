...
public class PowerUpSystemsTests
{
    ...
    public void PowerUpLifetimeSystem_RemovesExpiredPowerUp()
    {
        ...
        entity.AddComponent(world,
            new PowerUp { Type = PowerUpType.SpeedBoost, TimeToLiveSeconds = 1f, ElapsedSeconds = 0.9f });
    ...
    public void PowerUpEffectSystem_ChangesScoreMultiplier()
    {
        ...
        head.AddComponent(world, new ActivePowerUpEffect { Type = PowerUpType.DoubleScore, RemainingSeconds = 5f });

        var stats = world.CreateEntity();
        stats.AddComponent(world, new Score { Value = 0, FruitsEaten = 0, PowerUpsCollected = 0, ScoreMultiplier = 1 });
